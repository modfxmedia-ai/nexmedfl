...
import { PAYMENT_PLANS_INTRO, PAYMENT_PLANS_FAQS } from "@/lib/payment-content";

const RELATED_LINKS = [
  {
    label: "Financing Options",
    description: "Prefer a third-party lender instead? See how financing works.",
...
/**
 * /payment-plans/ page body — plain-language FAQ content about in-office
 * payment plans, the Cherry financing widget (per client request, embedded
 * on this page), and related-info cross-links. Styled with the site's
 * brand tokens.
 */
export function PaymentPlansPageBody() {
  return (
    <main>
      {/* Hero */}
      ...
      <section className="mx-auto max-w-4xl px-4 pb-20 sm:px-6 lg:px-8">
        ...
        <Reveal delay={0.05} className="mt-6 rounded-2xl border border-surface-border bg-white px-6 sm:px-8">
          <FaqAccordion items={PAYMENT_PLANS_FAQS} />
        </Reveal>
      </section>

      {/* Cherry financing calculator widget */}
        ...
        <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6 lg:px-8">
          <Reveal className="mx-auto max-w-2xl text-center">
            ...
            <p className="mt-4 text-base leading-relaxed text-ink-soft">
              Use the calculator below for a quick, no-obligation estimate of
              what a structured payment plan could look like for your care.
            </p>
          </Reveal>

          <Reveal delay={0.1} className="mt-10 overflow-hidden rounded-2xl border border-surface-border bg-white p-2 shadow-[0_24px_60px_-38px_rgba(20,121,232,0.3)] sm:p-4">
            <CherryWidget />
          </Reveal>
        </div>
      </section>
